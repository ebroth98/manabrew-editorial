...
export function ChatComponent({ channelId }: ChatComponentProps) {
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', sender: 'System', content: 'Welcome to OpenMagic!', timestamp: new Date(), type: 'system' },
  ]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-3 py-2 border-b flex items-center gap-2">
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{channelId} Chat</span>
        <span className="text-[10px] text-muted-foreground/50 bg-muted px-1.5 py-0.5 rounded-full">{messages.length}</span>
      </div>
