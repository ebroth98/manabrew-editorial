...
export const MODAL_CARD_THUMBNAIL = "w-[60px] h-[84px] rounded-md object-cover shrink-0 shadow-md" as const;

/** Larger card image used in modal bodies (e.g., CostModal, ChooseOptionalTriggerModal) */
export const MODAL_CARD_IMAGE = "w-[120px] h-[168px] rounded-lg object-cover shrink-0 shadow-md" as const;

/** Text input field used in filter/search inputs inside modals */
export const MODAL_INPUT = "w-full px-3 py-1.5 rounded-md border bg-background text-sm focus:outline-none focus:ring-1 focus:ring-primary" as const;
...
export const MODAL_FOOTER_BETWEEN = "flex justify-between items-center px-4 py-3 border-t bg-muted/10 rounded-b-xl gap-2" as const;

// ── Zone label ─────────────────────────────────────────────────────────────

export const ZONE_LABEL = "text-xs font-semibold text-muted-foreground uppercase tracking-wide px-1" as const;
...
/** Inactive tab state */
export const TAB_INACTIVE = "text-muted-foreground border-transparent hover:text-foreground" as const;
