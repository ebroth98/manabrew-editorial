...
const COLOR_INFO: Record<string, { symbol: string; bg: string; ring: string; text: string }> = {
  White: { symbol: "W", bg: "bg-amber-50 dark:bg-amber-100", ring: "ring-amber-300", text: "text-amber-900" },
  Blue:  { symbol: "U", bg: "bg-blue-100 dark:bg-blue-200", ring: "ring-blue-400", text: "text-blue-900" },
  Black: { symbol: "B", bg: "bg-gray-300 dark:bg-gray-400", ring: "ring-gray-600", text: "text-gray-900" },
  Red:   { symbol: "R", bg: "bg-red-100 dark:bg-red-200", ring: "ring-red-400", text: "text-red-900" },
  Green: { symbol: "G", bg: "bg-green-100 dark:bg-green-200", ring: "ring-green-500", text: "text-green-900" },
};
      ...
      <div ref={dialogRef} tabIndex={-1} className="outline-none" role="dialog" aria-modal="true">
        ...
        <div className="p-4 flex flex-wrap gap-3 justify-center">
          {validColors.map((color) => {
            const info = COLOR_INFO[color] ?? { symbol: color[0], bg: "bg-muted", ring: "ring-border", text: "text-foreground" };
            return (
              <button
                key={color}
                ...
                className={cn(
                  ...
                  info.bg, info.text, `hover:${info.ring}`,
                )}
              >
                <img
