...
// ── Text styles ───────────────────────────────────────────────────────
const statStyle = new TextStyle({ fontFamily: FONT, fontSize: 12, fontWeight: "bold", fill: 0xdddddd });
const manaCountStyle = new TextStyle({ fontFamily: FONT, fontSize: 9, fontWeight: "800", fill: 0xffffff });
...
export class PlayerSquarePanel implements PlayerPanel {
  ...
  constructor(theme: PixiThemeColors, options?: { anchorTop?: boolean }) {
  ...
  update(state: PlayerPanelState): void {
    ...
    const seatColor = t.playerColors[state.playerSeat];
    this.bgGfx.clear();
    ...
    if (state.isTargetable) {
      ...
      this.targetGfx.fill({ color: t.promptAction.attackAction, alpha: 0.15 });
      this.targetGfx.stroke({ color: t.promptAction.attackAction, width: 2, alignment: 0.5, alpha: 0.85 });
    }
    ...
    const cardRingHex = `#${t.cardRing.toString(16).padStart(6, "0")}`;
    ...
    const placeRadial = (
      ...
      const hex = highlight ? cardRingHex : (ICON_COLORS[cell.iconKey] ?? "#6b7280");
      ...
      cell.value.style.fill = highlight ? t.activeAction.active : 0xdddddd;
    ...
    const placeStat2 = (cell: StatCell, val: number, visible: boolean, highlight: boolean) => {
      ...
      if (!visible) return;
      const hex = highlight ? `#${t.activeAction.active.toString(16).padStart(6, "0")}` : (ICON_COLORS[cell.iconKey] ?? "#6b7280");
      applyIcon(cell.icon, cell.iconKey, hex);
