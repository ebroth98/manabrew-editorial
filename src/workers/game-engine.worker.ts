...
/**
 * URL of the unified card+token+edition archive. Generated by
 * `forge-cardset-archive`'s `build-cardset-archive` binary and emitted into
 * `public/wasm/` during `yarn build:web` / `yarn dev:web`.
 *
 * The `v2` suffix in the filename matches `ARCHIVE_FORMAT_VERSION` in the
 * Rust archive crate. Bumping the format bumps the filename, which is also
 * the Cache API key — old entries age out naturally.
 */
const CARD_ARCHIVE_URL = "/wasm/cardset.v4.rkyv";
const CARD_ARCHIVE_CACHE = "manabrew-card-archive-v4";
...
async function fetchCardArchive(silent: boolean): Promise<ArrayBuffer> {
  ...
  const cached = await cache.match(CARD_ARCHIVE_URL);
  if (cached) {
    if (!silent) postEvent("worker:init", { stage: "cached" });
    return cached.arrayBuffer();
  }

  const response = await fetch(CARD_ARCHIVE_URL);
  if (!response.ok) {
