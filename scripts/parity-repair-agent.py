#!/usr/bin/env python3
"""
Parity Repair Agent v0.2 — autonomous divergence fixer for the Forge engine port.
...
CLAUDE_MODEL = "sonnet"
MAX_BUDGET_USD = 5.0  # per attempt (not per cluster)
RECENCY_HOURS = 24
...
def create_pr(branch: str, cluster: dict, test_results: list[dict],
    ...
    body_lines.extend([
        ...
        "",
        "---",
        "*Generated by the Parity Repair Agent v0.2*",
        "",
...
def attempt_repair(cluster: dict, dry_run: bool = False) -> bool:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        for s in failures[:3]:
            ...
            _log(f"    → {'PASS' if result['passed'] else 'FAIL'}")
...
def main():
    parser = argparse.ArgumentParser(description="Parity Repair Agent v0.2")
    ...
    print("╔══════════════════════════════════════╗")
    print("║     Parity Repair Agent v0.2         ║")
    print("╚══════════════════════════════════════╝")
