...
pub fn compare(
    ...
    if rust.turn != java.turn {
        divs.push(divergence(index, turn, &phase, "turn", &rust.turn, &java.turn));
    }
    if rust.phase != java.phase {
        divs.push(divergence(index, turn, &phase, "phase", &rust.phase, &java.phase));
    }
    if rust.active_player != java.active_player {
        divs.push(divergence(
            index, turn, &phase, "active_player",
            &rust.active_player, &java.active_player,
        ));
    }
    if rust.game_over != java.game_over {
        divs.push(divergence(index, turn, &phase, "game_over", &rust.game_over, &java.game_over));
    }
    if rust.winner != java.winner {
        divs.push(divergence(index, turn, &phase, "winner", &format!("{:?}", rust.winner), &format!("{:?}", java.winner)));
    }
    if rust.stack != java.stack {
        divs.push(divergence(index, turn, &phase, "stack", &format!("{:?}", rust.stack), &format!("{:?}", java.stack)));
    }
...
fn compare_players(
    ...
    compare_name_list(divs, index, turn, phase, &format!("{}.graveyard", prefix), &rust.graveyard, &java.graveyard);
    compare_name_list(divs, index, turn, phase, &format!("{}.hand", prefix), &rust.hand, &java.hand);
    compare_name_list(divs, index, turn, phase, &format!("{}.exile", prefix), &rust.exile, &java.exile);

    // Battlefield comparison
    compare_battlefield(divs, index, turn, phase, &format!("{}.battlefield", prefix), &rust.battlefield, &java.battlefield);
}
...
fn compare_name_list(
    ...
    if rust != java {
        divs.push(divergence(
            index, turn, phase, field,
            &format!("{:?}", rust), &format!("{:?}", java),
        ));
    }
}
...
fn compare_battlefield(
    ...
    for i in 0..max {
        ...
        match (rust.get(i), java.get(i)) {
            (Some(rc), Some(jc)) => {
                if rc.name != jc.name {
                    divs.push(divergence(index, turn, phase, &format!("{}.name", card_prefix), &rc.name, &jc.name));
                }
                if rc.tapped != jc.tapped {
                    divs.push(divergence(index, turn, phase, &format!("{}.tapped", card_prefix), &rc.tapped, &jc.tapped));
                }
                if rc.power != jc.power {
                    divs.push(divergence(index, turn, phase, &format!("{}.power", card_prefix), &format!("{:?}", rc.power), &format!("{:?}", jc.power)));
                }
                if rc.toughness != jc.toughness {
                    divs.push(divergence(index, turn, phase, &format!("{}.toughness", card_prefix), &format!("{:?}", rc.toughness), &format!("{:?}", jc.toughness)));
                }
                if rc.damage != jc.damage {
                    divs.push(divergence(index, turn, phase, &format!("{}.damage", card_prefix), &rc.damage, &jc.damage));
                }
                if rc.summoning_sick != jc.summoning_sick {
                    divs.push(divergence(index, turn, phase, &format!("{}.summoning_sick", card_prefix), &rc.summoning_sick, &jc.summoning_sick));
                }
                if rc.counters != jc.counters {
                    divs.push(divergence(index, turn, phase, &format!("{}.counters", card_prefix), &format!("{:?}", rc.counters), &format!("{:?}", jc.counters)));
                }
                if rc.controller != jc.controller {
                    divs.push(divergence(index, turn, phase, &format!("{}.controller", card_prefix), &rc.controller, &jc.controller));
                }
            }
            (Some(rc), None) => {
                divs.push(divergence(index, turn, phase, &format!("{}.exists", card_prefix), &rc.name, &"<missing>"));
            }
            (None, Some(jc)) => {
                divs.push(divergence(index, turn, phase, &format!("{}.exists", card_prefix), &"<missing>", &jc.name));
            }
            (None, None) => {}
        }
    }
