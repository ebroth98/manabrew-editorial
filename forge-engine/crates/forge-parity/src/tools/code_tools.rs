...
/// Search Rust source files by regex pattern within `forge-engine/`.
///
...
mod tests {
    ...
    /// Get the workspace root (3 levels up from CARGO_MANIFEST_DIR for forge-parity).
    fn workspace_root() -> PathBuf {
        let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        // forge-engine/crates/forge-parity -> go up 3 levels to repo root
        manifest
            .parent()
            .and_then(|p| p.parent())
            .and_then(|p| p.parent())
    ...
    fn read_file_with_line_range() {
        ...
        let result = read_file(
            &root,
            "forge-engine/crates/forge-parity/src/tools/code_tools.rs",
            Some(1),
            Some(5),
        );
    ...
    fn list_files_finds_rs_files() {
        let root = workspace_root();
        let result = list_files(&root, "forge-engine/crates/forge-parity/src/tools/*.rs");
        assert!(result.contains("code_tools.rs"));
    }
}
