...
cargo build -p forge-parity --features storage

# With web dashboard (enables --serve, implies storage)
cargo build -p forge-parity --features serve
```

`forge-parity` is the single binary for parity infrastructure. CI queue client commands are exposed as a mode:
...
Talks to a running `--serve --ci` parity server from the same `forge-parity` binary.

```bash
cargo run -p forge-parity --features serve -- ci-client health --server http://localhost:8080
```
...
The parity dashboard can be run alongside the forge-server using Docker Compose:

```bash
# From forge-engine/crates/forge-server/
