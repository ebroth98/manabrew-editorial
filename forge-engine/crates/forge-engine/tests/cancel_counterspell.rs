...
fn test_target_type_filter() {
    ...
    assert!(!has_valid_spell_with_filter(&game, "Spell"), 
    ...
    assert_eq!(valid_filtered.len(), 1, "Should only have 1 spell after Spell filter");
    assert_eq!(valid_filtered[0], 0, "Remaining spell should be the creature spell (ID 0)");
    ...
    assert!(!has_valid_spell_with_filter(&game2, "Spell"), 
...
fn test_cancel_counters_creature_spell() {
    ...
    let mut cancel_sa = SpellAbility::new_simple(Some(cancel), p0, "SP$ Counter | TargetType$ Spell | ValidTgts$ Card");
    ...
    cancel_sa.target_restrictions = Some(target_restrictions::TargetRestrictions {
        ...
        target_kind: target_restrictions::parse_valid_targets("SP$ Counter | TargetType$ Spell | ValidTgts$ Card"),
    ...
    assert!(cancel_sa.target_restrictions.as_ref().unwrap().has_candidates(&game, p0), 
        "Cancel should have valid targets (the Grizzly Bears spell)");
    
    // Get valid targets
    let valid = target_restrictions::get_all_candidates_spells(&game);
    assert_eq!(valid.len(), 1, "Should have 1 valid target");
    assert_eq!(valid[0], creature_spell_id, "Target should be the creature spell");
    
    // Apply the TargetType$ filter - this should keep the creature spell
    let valid_filtered = target_restrictions::filter_spells_by_type(&game, &valid, "Spell");
    assert_eq!(valid_filtered.len(), 1, "Should still have 1 valid target after Spell filter");
    assert_eq!(valid_filtered[0], creature_spell_id, "Creature spell should still be targetable");
...
fn test_cancel_counters_noncreature_spell() {
    ...
    let valid_filtered = target_restrictions::filter_spells_by_type(&game, &valid, "Spell");
    assert_eq!(valid_filtered.len(), 1, "Should still have 1 valid target after Spell filter");
    assert_eq!(valid_filtered[0], instant_spell_id, "Instant spell should still be targetable");
