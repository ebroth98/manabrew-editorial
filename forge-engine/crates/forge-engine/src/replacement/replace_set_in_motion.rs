//! Replacement logic for `Event$ SetInMotion`.
//!
//! Mirrors Java `ReplaceSetInMotion.java` in `forge/game/replacement/`.
...
pub fn can_replace(
    ...
    if effect.event != ReplacementType::SetInMotion {
        return false;
    }
...
/// Mirrors Java `ReplacementHandler.executeReplacement()` for SetInMotion.
pub fn execute(
    effect: &ReplacementEffect,
