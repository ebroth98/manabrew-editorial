//! Replacement logic for `Event$ Learn`.
//!
//! Mirrors Java `ReplaceLearn.java` in `forge/game/replacement/`.
...
/// Mirrors Java `ReplaceLearn.canReplace()`.
pub fn can_replace(
    ...
    if effect.event != ReplacementType::Learn {
        return false;
    }
...
/// Mirrors Java `ReplacementHandler.executeReplacement()` for Learn.
pub fn execute(
    effect: &ReplacementEffect,
