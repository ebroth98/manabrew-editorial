...
impl CardState {
    ...
    pub fn get_state_name(&self) -> CardStateName { self.state_name }
    pub fn get_type_with_changes(&self) -> &CardTypeView { self.changed_type.as_ref().unwrap_or(&self.r#type) }
    pub fn update_types(&mut self) { self.changed_type = Some(self.r#type.clone()); }
    pub fn update_types_for_view(&self) {}
    pub fn get_type(&self) -> &CardTypeView { &self.r#type }
    pub fn add_type(&mut self, type_value: String) { self.r#type.add_type(&type_value); }
    pub fn add_types<I: IntoIterator<Item = String>>(&mut self, type_values: I) { for t in type_values { self.r#type.add_type(&t); } }
    pub fn set_type(&mut self, type_value: CardType) { self.r#type = type_value; }
    pub fn remove_card_types_with_sanitize(&mut self, _sanitize: bool) { self.r#type.core_types.clear(); self.r#type.subtypes.clear(); }
    pub fn get_mana_cost(&self) -> &ManaCost { &self.mana_cost }
    pub fn set_mana_cost(&mut self, mana_cost: ManaCost) { self.mana_cost = mana_cost; }
    pub fn calculate_perpetual_adjusted_mana_cost_for_state(&mut self) {}
    pub fn get_perpetual_adjusted_mana_cost(&self) -> &ManaCost { self.perpetual_adjusted_mana_cost.as_ref().unwrap_or(&self.mana_cost) }
    pub fn get_color(&self) -> ColorSet { self.color }
    pub fn add_color(&mut self, color: ColorSet) { self.color = self.color.union(color); }
    pub fn set_color(&mut self, color: ColorSet) { self.color = color; }
    pub fn get_oracle_text(&self) -> &str { &self.oracle_text }
    pub fn set_oracle_text(&mut self, oracle_text: String) { self.oracle_text = oracle_text; }
    pub fn get_functional_variant_name(&self) -> Option<&str> { self.functional_variant_name.as_deref() }
    pub fn set_functional_variant_name(&mut self, functional_variant_name: Option<String>) { self.functional_variant_name = functional_variant_name.filter(|s| !s.is_empty()); }
    pub fn get_flavor_name(&self) -> Option<&str> { self.flavor_name.as_deref() }
    pub fn set_flavor_name(&mut self, flavor_name: Option<String>) { self.flavor_name = flavor_name; }
    pub fn get_base_power(&self) -> i32 { self.base_power }
    pub fn set_base_power(&mut self, base_power: i32) { self.base_power = base_power; }
    pub fn get_base_toughness(&self) -> i32 { self.base_toughness }
    pub fn set_base_toughness(&mut self, base_toughness: i32) { self.base_toughness = base_toughness; }
    pub fn get_base_power_string(&self) -> Option<&str> { self.base_power_string.as_deref() }
    pub fn get_base_toughness_string(&self) -> Option<&str> { self.base_toughness_string.as_deref() }
    pub fn set_base_power_string(&mut self, value: Option<String>) { self.base_power_string = value; }
    pub fn set_base_toughness_string(&mut self, value: Option<String>) { self.base_toughness_string = value; }
    pub fn get_base_loyalty(&self) -> &str { &self.base_loyalty }
    pub fn set_base_loyalty(&mut self, value: String) { self.base_loyalty = value; }
    pub fn get_base_defense(&self) -> &str { &self.base_defense }
    pub fn set_base_defense(&mut self, value: String) { self.base_defense = value; }
    pub fn get_attraction_lights(&self) -> Option<&HashSet<i32>> { self.attraction_lights.as_ref() }
    pub fn set_attraction_lights(&mut self, attraction_lights: Option<HashSet<i32>>) { self.attraction_lights = attraction_lights; }
    pub fn get_cached_keywords(&self) -> Vec<&crate::keyword::keyword_instance::KeywordInstanceData> { self.cached_keywords.get_values() }
    pub fn set_cached_keywords(&mut self, collection: KeywordCollection) { self.cached_keywords = collection; }
    pub fn has_keyword_enum(&self, key: crate::keyword::keyword_instance::Keyword) -> bool { self.cached_keywords.contains_keyword(key) }
    pub fn get_intrinsic_keywords(&self) -> Vec<&crate::keyword::keyword_instance::KeywordInstanceData> { self.intrinsic_keywords.get_values() }
    pub fn has_intrinsic_keyword_state(&self, keyword: &str) -> bool { self.intrinsic_keywords.contains(keyword) }
    pub fn set_intrinsic_keywords(&mut self, _intrinsic_keywords: Vec<KeywordInterface>, _lki: bool) {}
    pub fn update_keywords_cache_for_state(&mut self) { self.cached_keywords = self.intrinsic_keywords.clone(); }
    pub fn add_intrinsic_keyword_with_init_traits(&mut self, keyword: String, _init_traits: bool) -> Option<KeywordInterface> {
        if self.intrinsic_keywords.add(&keyword) { Some(KeywordInterface::new(crate::keyword::Keyword::smart_value_of(&keyword), keyword)) } else { None }
    }
    pub fn add_intrinsic_keywords_for_state(&mut self, keywords: Vec<String>, init_traits: bool) -> bool {
        let mut changed = false;
        for k in keywords { changed |= self.add_intrinsic_keyword_with_init_traits(k, init_traits).is_some(); }
        changed
    }
    pub fn remove_intrinsic_keyword_for_state(&mut self, keyword: &str) -> bool { self.intrinsic_keywords.remove(keyword) }
    pub fn get_spell_abilities(&self) -> FCollectionView<SpellAbility> { self.abilities.clone() }
    pub fn get_mana_abilities(&self) -> FCollectionView<SpellAbility> { self.abilities.clone().into_iter().filter(|sa| sa.is_mana_ability).collect() }
    pub fn get_non_mana_abilities(&self) -> FCollectionView<SpellAbility> { self.abilities.clone().into_iter().filter(|sa| !sa.is_mana_ability).collect() }
    pub fn update_spell_abilities(&self, _new_col: &mut FCollection<SpellAbility>) {}
    pub fn get_land_trait_changes(&self) -> &LandTraitChanges { &self.land_trait_changes }
    pub fn get_intrinsic_spell_abilities(&self) -> Vec<SpellAbility> { self.abilities.clone().into_iter().filter(|sa| sa.is_activated || sa.is_trigger || sa.is_spell).collect() }
    pub fn get_first_ability(&self) -> Option<SpellAbility> { self.get_intrinsic_spell_abilities().into_iter().next() }
    pub fn get_first_spell_ability(&self) -> Option<SpellAbility> { self.get_non_mana_abilities().into_iter().next() }
    pub fn get_first_spell_ability_with_fallback(&self) -> Option<SpellAbility> { self.get_first_spell_ability().or_else(|| self.get_first_ability()) }
    pub fn get_aura_spell(&self) -> Option<SpellAbility> { self.aura_ability.clone() }
    pub fn has_spell_ability_id(&self, id: i32) -> bool { self.get_spell_abilities().iter().any(|sa| sa.source_trigger_id == Some(id as u32)) }
    pub fn add_spell_ability_state(&mut self, ability: SpellAbility) -> bool { self.abilities.push(ability); true }
    pub fn get_triggers(&self) -> FCollectionView<Trigger> { self.triggers.clone() }
    pub fn has_trigger_id(&self, id: i32) -> bool { self.get_triggers().iter().any(|t| t.id == id as u32) }
    pub fn add_trigger_state(&mut self, trigger: Trigger) -> bool { self.triggers.push(trigger); true }
    pub fn get_static_abilities(&self) -> FCollectionView<StaticAbility> { self.static_abilities.clone() }
    pub fn add_static_ability_state(&mut self, static_ability: StaticAbility) -> bool { self.static_abilities.push(static_ability); true }
    pub fn remove_static_ability_state(&mut self, _static_ability: StaticAbility) -> bool { false }
    pub fn get_replacement_effects(&self) -> FCollectionView<ReplacementEffect> { self.replacement_effects.clone() }
    pub fn add_replacement_effect_state(&mut self, replacement_effect: ReplacementEffect) -> bool { self.replacement_effects.push(replacement_effect); true }
    pub fn has_replacement_effect_id(&self, id: i32) -> bool { self.get_replacement_effect(id).is_some() }
    pub fn get_replacement_effect(&self, id: i32) -> Option<ReplacementEffect> { self.get_replacement_effects().into_iter().find(|r| false || id == 0 && r.params.is_empty()) }
    pub fn get_foil(&self) -> i32 { self.get_svar("Foil").and_then(|s| s.parse().ok()).unwrap_or(0) }
    ...
    pub fn copy(&self, host: Card, name: CardStateName, _lki: bool) -> CardState { CardState::new(host, name) }
    pub fn get_rarity(&self) -> CardRarity { self.rarity }
    pub fn set_rarity(&mut self, rarity: CardRarity) { self.rarity = rarity; }
    pub fn get_set_code(&self) -> &str { &self.set_code }
    pub fn set_set_code(&mut self, set_code: String) { self.set_code = set_code; }
    pub fn get_image_key(&self) -> &str { &self.image_key }
    pub fn set_image_key(&mut self, image_key: String) { self.image_key = image_key; }
    pub fn get_traits(&self) -> Vec<CardTraitBase> { Vec::new() }
    ...
    pub fn has_chapter(&self) -> bool { self.get_triggers().iter().any(|t| t.params.has("Chapter")) }
    pub fn get_final_chapter_nr(&self) -> i32 { 0 }
    pub fn get_manifest_up(&self) -> Option<SpellAbility> { self.manifest_up.clone() }
    pub fn get_cloak_up(&self) -> Option<SpellAbility> { self.cloak_up.clone() }
    pub fn get_ability_for_trigger(&self, svar: String) -> Option<SpellAbility> { self.ability_for_trigger.get(&svar).cloned() }
}
...
impl ITranslatable for CardState {
    fn get_translation_key(&self) -> String {
        self.flavor_name.clone().unwrap_or_else(|| self.name.clone())
    }
