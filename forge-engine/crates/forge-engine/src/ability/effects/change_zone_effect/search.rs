...
pub(super) fn resolve_each_search(
    ctx: &mut EffectContext, sa: &SpellAbility, each_spec: &str,
    zone_cards: &mut Vec<CardId>, chooser: PlayerId, _is_optional: bool,
) -> Vec<CardId> {
    let mut out = Vec::new();
    for clause in each_spec.split('&').map(str::trim).filter(|s| !s.is_empty()) {
        let candidates: Vec<_> = zone_cards.iter().copied()
            .filter(|&cid| matches_with_context(ctx, sa, cid, clause)).collect();
        ...
        let chosen = if candidates.len() == 1 {
            ...
            ctx.agents[chooser.index()].choose_single_card_for_zone_change(
                chooser, &candidates,
                sa.select_prompt().unwrap_or("Select card for zone change"), false,
            )
        };
...
pub(super) fn resolve_single_search(
    ctx: &mut EffectContext, sa: &SpellAbility, candidates: &[CardId],
    chooser: PlayerId, is_optional: bool,
) -> Vec<CardId> {
    if candidates.len() == 1 && !is_optional { return vec![candidates[0]]; }
    ctx.agents[chooser.index()].snapshot_state(ctx.game, ctx.mana_pools);
    ctx.agents[chooser.index()].on_library_peek(ctx.game, candidates);
    ctx.agents[chooser.index()].choose_single_card_for_zone_change(
        chooser, candidates,
        sa.select_prompt().unwrap_or("Select card for zone change"), is_optional,
    ).into_iter().collect()
}
...
pub(super) fn resolve_multi_search(
    ctx: &mut EffectContext, sa: &SpellAbility, candidates: &[CardId],
    chooser: PlayerId, change_num: usize, _is_optional: bool,
) -> Vec<CardId> {
    ...
    if diff_names || diff_cmc || diff_power || share_land || budget_cmc.is_some() || budget_power.is_some() {
        return resolve_constrained_multi(ctx, sa, candidates, chooser, max,
            diff_names, diff_cmc, diff_power, share_land, budget_cmc, budget_power);
    }
...
fn resolve_constrained_multi(
    ctx: &mut EffectContext, sa: &SpellAbility, candidates: &[CardId],
    chooser: PlayerId, max: usize,
    diff_names: bool, diff_cmc: bool, diff_power: bool, share_land: bool,
    budget_cmc: Option<i32>, budget_power: Option<i32>,
) -> Vec<CardId> {
    ...
    for _ in 0..max {
        ...
        let Some(chosen) = ctx.agents[chooser.index()].choose_single_card_for_zone_change(
            chooser, &remaining,
            sa.select_prompt().unwrap_or("Select card for zone change"), true,
        ) else { break };
        ...
        if share_land && required_land_types.is_empty() { required_land_types = land_types; }
        ...
        remaining.retain(|&cid| {
            ...
            cid != chosen
                ...
                && (!share_land || required_land_types.is_empty()
                    || get_land_subtypes(&c.type_line.subtypes).iter().any(|lt| required_land_types.contains(lt)))
        });
    }
    selected
}
...
pub(super) fn resolve_random_selection(
    ctx: &mut EffectContext, candidates: &[CardId], count: usize,
) -> Vec<CardId> {
...
pub(super) fn resolve_defined_player_choice(
    ctx: &mut EffectContext, sa: &SpellAbility, origin_zone: ZoneType, change_type: &str,
) -> Vec<CardId> {
    let controller = sa.activating_player;
    let def = sa.defined_player().unwrap_or("");
    let players: Vec<PlayerId> = if def.eq_ignore_ascii_case("Player") {
        (0..ctx.game.players.len()).map(|i| PlayerId(i as u32)).collect()
        ...
        resolve_defined_player_with_sa(def, sa, controller, ctx.game)
            .map(|pid| vec![pid]).unwrap_or_else(|| vec![controller])
    };

    let mut collected = Vec::new();
    for pid in players {
        let candidates: Vec<_> = ctx.game.cards_in_zone(origin_zone, pid).to_vec()
            .into_iter().filter(|&cid| matches_with_context(ctx, sa, cid, change_type)).collect();
        if candidates.is_empty() { continue; }
        if candidates.len() == 1 {
        ...
        if let Some(chosen) = ctx.agents[pid.index()].choose_single_card_for_zone_change(
            pid, &candidates, "Select card for zone change", false,
