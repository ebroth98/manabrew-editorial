...
pub fn resolve(ctx: &mut EffectContext, sa: &SpellAbility) {
    ...
    let type_category = sa.params.get("Type").cloned().unwrap_or_else(|| "Creature".to_string());
    ...
    let valid_types: Vec<String> = if let Some(vt) = sa.params.get("ValidTypes") {
        ...
        match type_category.as_str() {
            ...
            "Land" => vec![
                "Plains".into(), "Island".into(), "Swamp".into(),
                "Mountain".into(), "Forest".into(),
            ],
            _ => vec![
                "Artifact".into(), "Creature".into(), "Enchantment".into(),
                "Instant".into(), "Land".into(), "Planeswalker".into(), "Sorcery".into(),
            ],
        }
    };

    if valid_types.is_empty() { return; }

    let chosen = ctx.agents[controller.index()]
        .choose_type(controller, &type_category, &valid_types);
...
fn default_creature_types() -> Vec<String> {
    vec![
        "Advisor", "Angel", "Ape", "Archer", "Assassin", "Avatar",
        "Beast", "Berserker", "Bird", "Boar",
        "Cat", "Centaur", "Cleric", "Construct", "Crab",
        "Demon", "Dinosaur", "Djinn", "Dog", "Dragon", "Drake", "Druid", "Dwarf",
        "Elemental", "Elephant", "Elf",
        "Faerie", "Fish", "Fox", "Frog", "Fungus",
        "Giant", "Gnome", "Goblin", "God", "Golem", "Griffin",
        "Horror", "Human", "Hydra",
        "Illusion", "Imp", "Insect",
        "Jellyfish",
        "Knight",
        "Lizard",
        "Merfolk", "Minotaur", "Monk", "Mutant", "Myr",
        "Ninja", "Noble",
        "Ogre", "Orc", "Otter", "Ox",
        "Pegasus", "Phoenix", "Pilot", "Pirate", "Plant", "Praetor",
        "Rat", "Rebel", "Rogue",
        "Salamander", "Samurai", "Scout", "Serpent", "Shade", "Shaman", "Shapeshifter",
        "Skeleton", "Sliver", "Snake", "Soldier", "Sphinx", "Spider", "Spirit", "Squirrel",
        "Thopter", "Treefolk", "Troll", "Turtle",
        "Unicorn",
        "Vampire", "Vedalken", "Viashino",
        "Wall", "Warrior", "Werewolf", "Wizard", "Wolf", "Wolverine", "Wurm",
        "Zombie",
