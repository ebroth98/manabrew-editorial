...
//!
//! Disabled by default. Set `FORGE_ENGINE_PERF=1` to collect counters and
//! timings. Set `FORGE_ENGINE_PERF_TRACE=1` to emit every increment, or
//! `FORGE_ENGINE_PERF_REPORT_EVERY=N` to print a summary every N continuous
...
fn enabled_cell() -> bool {
    std::env::var_os("FORGE_ENGINE_PERF").is_some()
}
...
fn trace_enabled() -> bool {
    ...
    *TRACE_ENABLED.get_or_init(|| std::env::var_os("FORGE_ENGINE_PERF_TRACE").is_some())
}

fn report_every() -> Option<u64> {
    static REPORT_EVERY: OnceLock<Option<u64>> = OnceLock::new();
    *REPORT_EVERY.get_or_init(|| {
        std::env::var("FORGE_ENGINE_PERF_REPORT_EVERY")
            .ok()
